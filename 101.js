...
!function (e, t) {
    ...
    var h = function (n) {
        var e, d, b, o, i, h, f, g, w, u, l, T, C, a, E, v, s, c, y, k = "sizzle" + 1 * new Date, m = n.document, S = 0, r = 0, p = ue(), x = ue(), N = ue(), A = ue(), D = function (e, t) {
            ...
            $ = ":(" + I + ")(?:\\((('((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\")|((?:\\\\.|[^\\\\()[\\]]|" + W + ")*)|.*)\\)|)", F = new RegExp(M + "+", "g"), B = new RegExp("^" + M + "+|((?:^|[^\\\\])(?:\\\\.)*)" + M + "+$", "g"), _ = new RegExp("^" + M + "*," + M + "*"), z = new RegExp("^" + M + "*([>+~]|" + M + ")" + M + "*"), U = new RegExp(M + "|>"), X = new RegExp($), V = new RegExp("^" + I + "$"),
...
function huoq(){
     ...
     if (domain == "") {
   layer.msg("网址不能为空", function(){
});
        return false;
    }
        var load = layer.msg("获取网站信息中...", {
          icon: 16
          ,shade: 0.01
          });
            $.ajax({
                type: "POST",
                url: 'http://www.kihaxm.cn/api/title.php?url='+$("#domain").val(),
                dataType: "json",
                success: function (data) {
                    if(data.code == 1 ){
                      layer.msg(data.msg,{icon:6});
                        $("#name").val(data.title);
                        $("#gjc").val(data.keywords);
     ...
     var introduce= $("#introduce").val();
        var load = layer.msg("正在提交中...", {
          ...
          ,shade: 0.01
           ...
           data : {"name":name,"sortname":sortname,"domain":domain,"gjc":gjc,"introduce":introduce},
		cache:false, //不缓存此页面 
   dataType : 'json',
	timeout:10000,
		success : function(data) {					  
